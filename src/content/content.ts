...
export const sortByUpdated = (notes) =>
  notes.sort((a, b) => new Date(b.data.updated).getTime() - new Date(a.data.updated).getTime())
export const sortByStartDate = (notes) =>
  notes.sort((a, b) => new Date(b.data.startDate).getTime() - new Date(a.data.startDate).getTime())

export const getTopics = (collections) => {
  return Object.entries(
    _.countBy(collections.flatMap((collection) => collection.flatMap(({ data }) => data.topics))),
  )
}
