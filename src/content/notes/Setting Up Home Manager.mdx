...
The first thing our Home Manager [[Nix Flake]] needs is a _input_ to get packages from. We're probably going to use some packages, so we need [[nixpkgs]], and then we're going to use `home-manager`, too:
...
Before we go any further, it's time give us some more safety while trying things out.
