...
It doesn't make any sense that we're limited to the current state the files we're working on are in. We should be able to seamlessly jump through the history of _every file there is_.
...
Computers are magic pieces of determinism. When there aren't any cosmic rays flipping bits (citation needed), everything we do could, in theory, work deterministically (provided we throw enough Math at it).
