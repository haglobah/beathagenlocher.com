...
And a file browser window is just this:

```
folder path -file browser-> folder view
```

When we go through with this, a big chunk (except for the part at the boundary where effects should happen) of our program is just [[Function Composition]].
...
This is okay (and still way better than a ton of statements, I'd say), but it loses the nice property of statements that it's not nested.

Also, when we try to read and understand the code, we're probably going to read it from right to left (while mentally 'piping' through the arguments in our heads).
...
export const main = (request) => {
  return pipe(request, parseHeaders, validateRequest, formatResponse)
}
```
...
- Pipelines need their functions to be functions of only one argument (in 'effect', for example), or they need to make a choice on where to splice the argument into the function call. Elixir always splices it as the first argument, while Lisps aren't decided on that (they decide per-case), and normally have both a `->` (inserts as the first argument) and `->>` (inserts as the last argument), as well as `-as->` variants, which you can call like this:
