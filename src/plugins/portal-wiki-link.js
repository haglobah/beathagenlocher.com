...
function wikiLink() {
  ...
  return {
    text:
      ((_text = {}),
      _defineProperty(_text, codes.leftSquareBracket, wikiLinkConstruct),
      _defineProperty(_text, codes.exclamationMark, wikiLinkConstruct),
      _text),
  }
}
...
function fromMarkdown() {
  ...
  function exitWikiLink(token) {
    ...
    this.exit(token)
    var wikiLinkWithHeadingPattern =
    ...
    if (isEmbed) {
      ...
      if (!isSupportedFormat) {
        ...
        var hasDimensions = alias && /^\d+(x\d+)?$/.test(alias)
        // Take the target as alt text except if alt name was provided [[target|alt text]]
        var altText = hasDimensions || !alias ? target : alias
        ...
        if (hasDimensions) {
          var _getImageSize = getImageSize(alias),
      ...
      wikiLink.data.hName = wikiLink.data.exists
        ? 'innerlink'
        : 'innerlinkempty'
      wikiLink.data.hProperties = {
        className: classNames,
...
var defaultPathToPermalinkFunc = function defaultPathToPermalinkFunc(
  filePath,
  markdownFolder,
) {
  var permalink = filePath
