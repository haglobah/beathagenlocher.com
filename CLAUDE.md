...
- **books** - The book library, synced from Zeneca (`just books` runs `scripts/sync-books.ts`). Frontmatter is machine-generated once; the MDX body is hand-written notes and is never overwritten by a re-sync. `description` starts as the publisher blurb (rewrite freely) and `recommendations` is a hand-incremented count of recommendations to friends, shown as a dot list. Covers live in `src/assets/books/`. Filenames must avoid glob characters (`?*[]{}()!`) — the Astro glob loader silently fails on them.
